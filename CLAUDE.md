...
Python version is managed by `.python-version` (3.13). Dependencies are managed with `uv`:
...
A `git-ops` agent is configured in `.claude/agents/git-ops.md`. It handles basic Git operations — commit, push, and pull — writing meaningful, logically-split commit messages and always using the machine's configured Git identity (never hardcoded). It never force-pushes or discards work without explicit confirmation.
